...
public class SqliteContactDAO implements INinjaContactDAO {
        ...
        try (PreparedStatement ps = connection.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
            ...
            try (ResultSet rs = ps.getGeneratedKeys()) {
                if (rs.next()) {
                    ...
                    // ✅ 初始化该用户的 Goals 和 Statistics 表
                    safeInitUserData(ninjaUser.getId());
                    recalcUserStatistics(ninjaUser.getId());
                    System.out.println("✅ Initialized Goals & Stats for new user ID=" + ninjaUser.getId());
                }
            }
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    public void recalcUserStatistics(int userId) {
        try {
            ...
            // 🟢 同时更新 TotalStars 与 Belt，避免 CHECK 约束错误
            String update = """
            UPDATE Statistics
